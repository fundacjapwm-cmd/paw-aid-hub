...
const ONas = () => {
  ...
  const services = [{
    icon: Package,
    label: "Karma i akcesoria"
  }, {
    icon: Scissors,
    label: "Sterylizacja"
  }, {
    icon: Stethoscope,
    label: "Opieka weterynaryjna"
  }, {
    icon: PawIcon,
      ...
      <main className="relative z-10">
        ...
        <section className="relative pt-8 pb-12 md:py-12 min-h-[400px] md:min-h-[500px] lg:min-h-[600px] overflow-hidden flex items-center">
          ...
          <div className="absolute inset-0 z-0">
            <img src={heroKittens} alt="" className="w-full h-full object-cover" />
          ...
          <div className="absolute inset-0 z-[1] pointer-events-none opacity-5">
            <img src={decorativePaws} alt="" className="absolute top-10 right-10 w-64 md:w-80 lg:w-96" />
          </div>
        ...
        <section className="bg-primary py-5 overflow-hidden relative z-20">
          <div className="flex animate-marquee whitespace-nowrap">
            {[...services, ...services].map((service, index) => <div key={index} className="flex items-center mx-8 md:mx-10">
            ...
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              ...
              <div className="flex justify-center lg:justify-start">
                <div className="w-full max-w-md aspect-square flex items-center justify-center bg-muted/50 rounded-[3rem] p-8 border border-border/30">
                  <img src={logo} alt="Pączki w Maśle - logo" className="w-full h-full object-contain" />
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-muted-foreground leading-relaxed mb-4 text-base md:text-lg">
                  Fundacja Pączki w Maśle powstała z miłości do zwierząt i potrzeby działania tam, gdzie jest to najbardziej potrzebne. Współpracujemy ze schroniskami, domami tymczasowymi i organizacji na terenie całej Polski.

Nasza siła to wieloletnie doświadczenie, pasja do zwierząt oraz ludzie, którzy z sercem dbają o zwierzęta w potrzebie.
        ...
        <section className="py-16 md:py-24 bg-muted/50 relative">
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {scopeItems.map((item, i) => <div key={i} className={`rounded-3xl p-6 transition-all duration-500 ${activeServiceIndex === i ? 'bg-primary text-primary-foreground shadow-lg scale-[1.02]' : 'bg-background border border-border/50 shadow-sm'}`}>
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${activeServiceIndex === i ? 'bg-primary-foreground/20' : 'bg-muted'}`}>
                    <item.Icon className={`w-7 h-7 ${activeServiceIndex === i ? 'text-primary-foreground' : 'text-primary'}`} />
                  </div>
                  <h3 className={`font-semibold mb-2 text-lg leading-tight ${activeServiceIndex === i ? 'text-primary-foreground' : 'text-foreground'}`}>
                    {item.title}
                  </h3>
                  <p className={`text-sm leading-relaxed ${activeServiceIndex === i ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>
                    {item.desc}
                  </p>
                </div>)}
            </div>
        ...
        <section className="py-16 md:py-24 bg-background relative">
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {helpSteps.map((item, i) => <div key={i} className="text-center">
                  <div className="relative mx-auto w-28 h-28 md:w-32 md:h-32 mb-6 flex items-center justify-center">
                    ...
                    {/* Icon */}
                    <item.Icon className={`relative z-10 w-16 h-16 ${item.color}`} />
                    {/* Step number badge */}
