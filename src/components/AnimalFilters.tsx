...
import { Filter, X, Building2, MapPin, ChevronDown, ChevronUp } from "lucide-react";
...
const AnimalFilters = ({ onFilterChange }: FiltersProps) => {
  ...
  const renderFilters = (isMobile: boolean) => (
    <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4'}`}>
